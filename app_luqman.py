...
def task():
    if request.method == "GET":
        ...
        #print(choice)
        rows = conn.execute(text(f"SELECT product_code, product_name FROM product_list ORDER BY product_code ASC"))
        prods = rows.all()
...
def Create_Quotation():
    if request.method == "POST":
        ...
        rows = conn.execute(text(f"SELECT * FROM product_list WHERE product_code = '{code}'"))
        prod = rows.all()
...
def single_price():
    if request.method == "POST":
        ...
        conn.execute(text(f'UPDATE product_list SET Price = "{price}" WHERE Product_Code = "{code}"'))
        conn.commit()
        return render_template("edit_prices.html")
    return render_template("single_product.html")
...
def add_product():
    if request.method == "POST":
        ...
        conn.execute(text(f'INSERT INTO product_list(Product_Code, Product_Name, Price, Description, Image_Directory) VALUES ("{code}", "{name}", "{price}", "{description}", "{image}")'))
        conn.commit()
        return render_template("admin_options.html")
    return render_template("add_product.html")
...
def update_password(user_name: str, new_password) -> bool:
    ...
    new_hash = generate_password_hash(new_password, method="pbkdf2", salt_length=16) # TODO: Security FLAW
    query = f"UPDATE TABLE users.hash = '{new_hash}' WHERE name = '{user_name}'" # TODO: Something like this. fix it
    response = conn.execute(text(query))
    return response.status == 200

def convert_url_to_qr_code(url: str):
    ...
